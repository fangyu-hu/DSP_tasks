...
%% 设数字低通滤波器的通带截至频率为 cetaP =0.5*pi;
cetaP =0.2*pi;
beta = cos((wp2+wp1)/2)/cos((wp2-wp1)/2)
k = cot((wp2-wp1)/2)*tan(cetaP/2)
alpha1 = (-2*beta*k)/(1+k)
alpha2 = (k-1)/(1+k)
cetaS=log(-(exp(-j*2*ws2)+alpha1*exp(-j*ws2)+alpha2)/(alpha2*exp(-j*2*ws2)+alpha1*exp(-j*ws1)+1))/(-j)
...
[bz,az] = lp2bpz(Bz,Az,alpha1,alpha2)  %%（自编函数）
%% 画出所设计数字滤波器的幅频特性，并检测Rp，As是否满足设计指标
[Rpp,Ass] = freqzn(bz, az, [wp1,wp2]/pi, [ws1,ws2]/pi, Rp, As, 'band') %%（自编函数）